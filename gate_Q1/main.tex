...
\begin{enumerate}
	...
	\begin{align}
		y_{k} -\alpha_{1} \log_{e}k &< \alpha_{0} \\%\quad \forall k|y_{k} - \alpha_{1}\log_{e}k \geq 0\\
		\therefore \tilde{\alpha_{0}} &= \underset{\brak{k|y_{k} - \alpha_{0}-\alpha_{1}\log_{e}k < 0}}{min} \brak{y_{k} - \alpha_{1}\log_{e}k}\\
		y_{k} - \alpha_{0} &< \alpha_{1} \log_{e}k \\%\quad \forall k|y_{k} -\alpha_{0}- \alpha_{1}\log_{e}k < 0\\
