...
\node (D) at (\p,0)[point,label=below:$D$] {};
\node (E) at (2.63,2.98)[point,label=above right:$E$] {};
\node (F) at (1.9,2.79)[point,label= left:$F$] {};
...
\draw [name path=line1](B)-- node[right] {$\textrm{g}$}(E);
\draw [name path=line2](C) -- node[left] {$\textrm{k}$}(F);

%Labeling orthocenter
\path[name intersections={of=line1 and line2,by=H}];
\node[point,inner sep=1pt,label=-85:$H$] at (H) {};
...
\tkzMarkRightAngle[fill=blue!20,size=.2](C,F,A)
